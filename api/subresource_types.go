...
type SubResourceAdapter interface {
	...
	// RecreateWhenTargetReplaced returns true if subresource should be recreated
	// when the target is replaced (e.g., during XSet update or external target deletion).
	// When true, subresources are deleted and recreated instead of being reused.
	RecreateWhenTargetReplaced(xset XSetObject) bool
