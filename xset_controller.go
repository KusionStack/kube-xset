...
func (r *xSetCommonReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	// Handle deletion before constructing revisions to avoid namespace termination issues.
	// When a namespace is being terminated, ConstructRevisions fails because it can't create
	// new ControllerRevision objects. We need to handle the deletion flow directly.
	if instance.GetDeletionTimestamp() != nil {
		if err := r.releaseResourcesForDeletion(ctx, instance, newStatus); err != nil {
			logger.Error(err, "failed to release resources for deletion")
			synccontrols.AddOrUpdateCondition(newStatus, api.XSetTerminating, err, "ReleaseResourcesFailed", err.Error())
		}
		if err := r.updateStatus(ctx, instance, newStatus); err != nil {
			return ctrl.Result{}, fmt.Errorf("fail to update status of %s %s: %w", kind, req, err)
		}
		return ctrl.Result{}, nil
	}
