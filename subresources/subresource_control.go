...
type SubResourceControl interface {
	...
	// Only deletes resources for adapters where RecreateWhenTargetReplaced returns true.
...
func setUpCacheForAdapters(cache cache.Cache, scheme *runtime.Scheme, adapters []api.SubResourceAdapter, controller api.XSetController) error {
	for _, adapter := range adapters {
		...
		if !ok {
			return fmt.Errorf("failed to set up cache for adapter GVK %s: type %T does not implement client.Object", gvk, obj)
		}
		if err := cache.IndexField(context.TODO(), clientObj, FieldIndexOwnerRefUID, func(object client.Object) []string {
...
func (sc *RealSubResourceControl) filterByTarget(existing []SubResourceState, target client.Object) []SubResourceState {
	targetID, exists := sc.labelAnnoMgr.Get(target, api.XInstanceIdLabelKey)
	if !exists || targetID == "" {
		return nil
	}
	var result []SubResourceState
	for _, state := range existing {
		if resourceID, ok := sc.labelAnnoMgr.Get(state.Object, api.XInstanceIdLabelKey); ok && resourceID == targetID {
...
func (sc *RealSubResourceControl) hasRecreateOnUpdateResources(xset api.XSetObject, target client.Object, existing []SubResourceState) bool {
	for _, state := range sc.filterByTarget(existing, target) {
		if state.Adapter != nil && state.Adapter.RecreateWhenTargetReplaced(xset) {
			return true
		}
	}
	return false
...
func (sc *RealSubResourceControl) createResourcesForAdapter(ctx context.Context, xset api.XSetObject, target client.Object, existing []SubResourceState, targetID string, adapter api.SubResourceAdapter) error {
	...
	for _, template := range templates {
		...
		if existing, ok := existingByTemplateName[template.Name]; ok {
			existingHash, hasHash := sc.labelAnnoMgr.Get(existing.Object, api.SubResourceTemplateHashLabelKey)
			// If no hash on existing resource, treat as stale and recreate
			if !hasHash || existingHash != template.Hash {
				// Delete old resource before creating new one (hash mismatch or missing hash)
				...
				// Hash matches
				if adapter.RecreateWhenTargetReplaced(xset) {
					// Adapter wants fresh resource on target replacement (e.g., PVC on pod recreate).
					// Delete existing and fall through to create new.
					if err := sc.deleteResource(ctx, xset, existing.Object, gvk); err != nil && !apierrors.IsNotFound(err) {
						return fmt.Errorf("failed to delete stale %s %s for target recreation: %w", gvk.Kind, existing.Object.GetName(), err)
					}
				} else {
		...
		// If decorator set GenerateName, clear Name so GenerateName takes effect.
		// This handles templates that may have placeholder names (which would override GenerateName).
		// Note: If a user intentionally sets both Name and GenerateName, this clearing is intentional
		// because GenerateName indicates the user wants a generated name, not a specific name.
		// If you want a specific name, set only Name (not GenerateName) in DecorateResource.
		if resource.GetGenerateName() != "" {
			resource.SetName("")
		}

		// Set GenerateName if neither Name nor GenerateName is set by decorator
...
// IsTargetTemplateChanged returns true if any subresource template changed for target,
// or if the target has resources that need recreation on update (when isUpdatedRevision is false).
func (sc *RealSubResourceControl) IsTargetTemplateChanged(xset api.XSetObject, target client.Object, existing []SubResourceState, isUpdatedRevision bool) (bool, error) {
	targetID, exist := sc.labelAnnoMgr.Get(target, api.XInstanceIdLabelKey)
	if !exist {
		return false, nil
	}
	...
	for _, adapter := range sc.adaptersByGVK {
		changed, err := sc.isAdapterTemplateChanged(xset, targetID, adapter, existing)
		if err != nil {
			return false, err
		}
		if changed {
	...
	if !isUpdatedRevision && sc.hasRecreateOnUpdateResources(xset, target, existing) {
		return true, nil
	}
...
// false positives). Missing resource detection is handled by RecreateWhenTargetReplaced
// and the update flow's DeleteTargetRecreateResources.
func (sc *RealSubResourceControl) isAdapterTemplateChanged(xset api.XSetObject, targetID string, adapter api.SubResourceAdapter, existing []SubResourceState) (bool, error) {
	gvk := adapter.Meta()
	...
	for _, state := range existing {
		...
		templateName, exist := sc.labelAnnoMgr.Get(state.Object, api.SubResourceTemplateLabelKey)
		if !exist {
			continue
		}
...
func (sc *RealSubResourceControl) CheckAllowIncludeExclude(ctx context.Context, xset api.XSetObject, target client.Object, fn CheckAllowFunc) (bool, error) {
	...
	for _, state := range targetResources {
		if allowed, reason := fn(state.Object, ownerName, ownerKind, sc.labelAnnoMgr); !allowed {
